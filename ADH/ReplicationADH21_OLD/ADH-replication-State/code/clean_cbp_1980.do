...
In this code, we take the raw CBP 1980 cleaned from Eckert, Fort, Schott and 
Yang (2021) to prepare the employment by sector dataset. 

Input files/base codes: 

1. other/cbp1980_imputations.do Replication package code to impute employment 
values to each county. 
2. other/subfile_sic87dd.do Replication package code to adjust the SIC 87 codes 
to the criteria applied in Dorn (2009). We cannot convert from SIC 77 to SIC 87 
because of the code splits and reverse paths in the bridge. Hence, we assume 
they're equal just for 1980. 
3. data/0-raw_data/cbp80st.txt EFCY CBP 1980 raw cleaned data at the state level. 

Output files: 
1. data/1-intermediate_data/cbp1980.dta Clean file with state, sector and 
employment.

*/

* Import the dataset
import delimited "../data/0-raw_data/cbp80st.txt", bindquote(strict) clear

* Rename variables for consistency
rename fipstate statefip
rename siccode1 siccode
rename tempmm emp

* Keep variables of interest
keep statefip siccode emp

* Drop aggregate sectors (cbp1980_imputations.do)
#delimit ;
replace siccode="." if siccode=="----" | siccode=="07--" | siccode=="10--" 
| siccode=="15--" | siccode=="19--" | siccode=="40--" | siccode=="50--"
| siccode=="52--" | siccode=="60--" | siccode=="70--" | siccode=="99--" 
| siccode=="098/" | siccode=="149/" | siccode=="179/" | siccode=="399/"
| siccode=="497/" | siccode=="519/" | siccode=="599/" | siccode=="679/" 
| siccode=="899/"; 
#delimit cr

drop if siccode == "." | siccode == "0000"

* Rename variables for consistency
rename siccode sic87
...
* Save final dataset
save ../data/1-intermediate_data/cbp1980.dta, replace
