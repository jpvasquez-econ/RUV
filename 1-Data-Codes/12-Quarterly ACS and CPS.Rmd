...
## Quarterly
cps_test <- read.csv(paste("/Users/yyh/Documents/Github/RUV/1-Data-Codes/1-Intermediate_Processed_Data/cps_2_quarterly.csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
## Yearly
cps_test <- read.csv(paste("/Users/yyh/Documents/Github/RUV/1-Data-Codes/1-Intermediate_Processed_Data/cps_2.csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
temp_case1  <- temp_case1  %>% 
  group_by(year, state_dest, sector_ori)  %>%
  mutate(sum_sec = sum(n_cps))  %>% 
  ungroup()  %>% 
  mutate(share_sec = n_cps/sum_sec)  %>% 
  mutate(flag = ifelse(sector_ori == sector_dest, 1, 0))
