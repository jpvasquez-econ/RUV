...
# X_i,US from census + proportionality
  check.imports.state.country.prop = imports.state.country.prop[imports.state.country.prop$sector==14, 3:dim(imports.state.country.prop)[2]]
...
if( is.na(sum(check))== TRUE) {
  ...
  sec=c(14)
  for (k in sec) {
    check.imports.state.country.prop = imports.state.country.prop[imports.state.country.prop$sector==k, 3:dim(imports.state.country.prop)[2]]
...
stopifnot(norm(tau.mod-tau.mod2)<0.001)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# NEW ATTEMPT: 8. CREATING X_J FOR STATES
...
# total sum
LHS.3<-t(P.s)
LHS<-as.matrix(rbind(LHS.1,LHS.2,LHS.3));
...
RHS.3 <- WIOD.base %>%
  ...
  select(-importer, -importer_country, -sector, -year)
RHS.3<-sum(RHS.3);
RHS<-matrix(rbind(RHS.1,t(RHS.2),RHS.3));
##### system to minimize
fr <- function( x ) {   
  #consumption of US from US matches WIOD
    entry.1<-(LHS.1%*%x/RHS.1-1);
  #matches imports from each country  
    entry.2<-0*(LHS.2%*%x/RHS.2-1);
  #total imports
    entry.3<-(sum(LHS.2%*%x)/sum(RHS.2)-1);
  #total consumption of US  
  #  entry.4<-0*(LHS.3%*%x/RHS.3-1);
  norm(rbind(entry.1, entry.2, entry.3))
}
...
as.numeric( optimal.model$value );
