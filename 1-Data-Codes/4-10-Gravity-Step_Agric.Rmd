...
sum_country02=c()
relevant=c(1:13) #13 doesnt refer to sector 13 but sector 14 in position 13
for (i in relevant) {
  q1=t(df.WIOD1[i,])
...
df.CFS <- df.CFS %>%
  filter(sector<=12 | sector==14)
e1$sector = df.CFS$sector[1:650]
e1$importer = df.CFS$importer[1:650]
...
#modifying exp vector since states' expenditure was NA
#X_j, j \notin US
...
X_j.mod <- rbind(X_j.mod.s, X_j.mod.c)

#Comparing X_j^hat with X_j for countries
h=print(sum(exp.vector[(n.s+1):(n.s+n.c)]-X_j.mod.c))

X_j.mod <- rbind(X_j.mod.1, matrix(exp.vector[(n.s+1):(n.s+n.c)]) )
