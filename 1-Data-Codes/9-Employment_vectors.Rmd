...
gravity_employment_1 <- gravity_employment %>%
  ...
  mutate(I_isjk = ifelse(state_ori==state_dest & sector_ori==sector_dest, 1, 0)) %>%
...
Sys.time()
# running regression
fit <- felm(as.formula(paste(dep_var," ~ log_dist + I_isjk +", paste(I_jk_dumm, collapse = "+"),"+", paste(I_sk_dumm, collapse = "+"),"| I_is")),
...
col_names <- col_names %>%
  ...
  mutate(coef = ifelse(substr(variable, 1,5)=="I_is_" | substr(variable, 1,5)=="I_jk_", 0, coef))
