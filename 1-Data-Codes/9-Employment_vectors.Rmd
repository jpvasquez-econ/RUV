...
6. `2-Final_Data/L_1999.xlsx`
7. `2-Final_Data/mu_1999.xlsx`
...
#flows
L_1999 <- read_excel(paste("2-Final_Data/L_1999.xlsx", sep=""))
...
L_flow <- melt(L_flow, id.vars=c("state_ori", "sector_ori"), variable.name="state_sector", value.name = "L_flow")
L_flow <- L_flow %>% 
...
#adding distances
L_flow <- L_flow %>% 
  select(state_ori, sector_ori, state_dest, sector_dest, L_flow) %>% 
  mutate(year = 1999) %>%
  left_join(distances, by=c("state_ori"="iso_o", "state_dest"="iso_d")) %>%
  select(year, state_ori, sector_ori, state_dest, sector_dest, dist, L_flow)

#saving
write_xlsx(L_flow, path = "2-Final_Data//Gravity_employment.xlsx")
...
gravity_employment <- gravity_employment %>%
  ...
  mutate(tot_ori = sum(L_flow)) %>%
  ungroup() %>%
  mutate(L_flow = L_flow/tot_ori) %>%
  select(-tot_ori)
...
#saveRDS(gravity_employment_1, file = "gravity_bss.rds")
...
gravity_employment_2 <- gravity_employment_no_dumm %>% select(-year, -state_ori, -sector_ori, -state_dest, -sector_dest, -dist, -L_flow, -I_is, -I_jk, -I_sk, -log_dist, -I_same_st_sec, -I_same_st, -I_same_sec)
