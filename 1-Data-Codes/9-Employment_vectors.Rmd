...
2) This code produces the level migration flows for state-sector for options 1-5, including the distances between states. 

3) This code runs regression on 
...
I_sk_dumm <- gravity_employment_1 %>%
  filter(sector_dest != 14 & sector_ori != 14) %>%
...
Sys.time()
# running regression
fit <- felm(as.formula(paste(dep_var," ~ log_dist +", paste(I_jk_dumm, collapse = "+"),"+", paste(I_sk_dumm, collapse = "+"),"| I_is")),
...
write.table(coefs, paste("1-Intermediate_Processed_Data/reg_mig_gravity_",dep_var,".csv", sep=""), sep=",", row.names = FALSE)

coefs <-read.csv(paste("1-Intermediate_Processed_Data/reg_mig_gravity_",dep_var,".csv", sep=""))

gravity_employment_2 <- gravity_employment_1 %>% select(-year, -state_ori, -sector_ori, -state_dest, -sector_dest, -dist, -L_flow_op1, -L_flow_op2, -L_flow_op3, -L_flow_op4, -L_flow_op5, -I_is, -I_jk, -I_sk)
...
col_names <- col_names %>%
  ...
  mutate(coef = ifelse(substr(variable, 1,4)=="I_is" | substr(variable, 1,4)=="I_jk", 0, coef))
matrix <- as.matrix(gravity_employment_2)
vec <- as.matrix(col_names$coef)  
Z <- data.frame(matrix %*% vec)
colnames(Z) <- c("Z")
...
write.table(Z, paste("2-Final_Data/Z_mig_gravity_",dep_var,".csv", sep=""), sep=",", row.names = FALSE)
