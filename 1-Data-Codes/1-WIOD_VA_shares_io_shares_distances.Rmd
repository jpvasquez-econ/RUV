...
#Drop sectors with row_item equal to 0 or NA 
final2  <- final2 [!is.na(final2$row_item), ] %>% filter(row_item != 0)
#Collapsing data
final2 <- final2  %>%
    group_by(year, col_country, row_item) %>%
...
#keeping manufacturing only 
final2   <- final2   %>% filter(!(row_item %in% c(13, 14)))
...
# Drop sectors without a row_item and collapse by row_item 
calibration_target<- calibration_target[!is.na(calibration_target$row_item), ] %>% filter(row_item != 0)
...
calibration_target <-calibration_target |>
  as.data.frame() |> reshape(idvar=c("year", "row_item"), timevar="col_country", direction="wide")
#This sould give the same than the stata code if we drop the sectors 13 and 14. 
#I am going to drop them but this can be change latter
calibration_target  <- calibration_target  %>% filter(!(row_item %in% c(13, 14)))
...
#time-set
calibration_target <- xts(calibration_target, order.by = as.POSIXct(calibration_target$year))
...
where $R_{i,k}$ denotes total revenues in sector $k$ of country $i$ and $X_{j,sk} = \sum_i X_{ij,sk}$ is the total purchases that sector k in region j makes from sector s.
