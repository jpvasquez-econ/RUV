...
#yr_list <- c("97", "98","99", "00", "01", "02", "03", "04", "05", "06", "07", "08", "09")
...
base <- base %>%
  ...
  #mutate(n_cps = sum(person_weight_1)) %>%
  mutate(n_cps = sum(person_weight_2)) %>%
  ungroup() %>%
...
#   ungroup() %>%
#   mutate(n_cps = ifelse(tot_within_state < 10 & state_ori==state_dest & sector_ori==sector_dest, tot, n_cps)) %>%
#   mutate(n_cps = ifelse(tot_within_state < 10 & (state_ori!=state_dest | sector_ori!=sector_dest), 0, n_cps)) %>%
