...
temp_LFP <- rbind(temp_LFP, temp_LFP, temp_LFP, temp_LFP, temp_LFP, temp_LFP, temp_LFP, temp_LFP, temp_LFP)
...
ACS_PUMS <- ACS_PUMS %>%
  ...
  filter( is.na(AGEP) == TRUE  | (AGEP >= 16)) %>% # age
...
tu <- acs %>%
  filter(year==1999) %>%
  mutate(w = ifelse(state_dest==state_ori, 1, 0)) %>%
  group_by(w) %>%
  mutate(n_acs = sum(n_acs)) %>%
  ungroup() %>%
  distinct(w, .keep_all = TRUE)
print(tu$n_acs[2]*100/(tu$n_acs[1]+tu$n_acs[2]))
...
cps <- cps %>%
  ...
  mutate(person_weight_1 = as.numeric(substr(V1, start=846, stop=855))) %>% #weight final
  mutate(person_weight_2 = as.numeric(substr(V1, start=593, stop=602))) %>% #weight longitude
  ...
  #mutate(employ_status_back = employ_status) %>%
...
####borrar
hola_1 <- cps_1 %>%
  filter(sector != 0 & year==2002) %>%
  #filter(person_weight_1!= -1) %>%
  filter(person_weight_2!= -1) %>%
  group_by(year, month, sector) %>%
  #mutate(n_cps = sum(person_weight_1/(10000*1000))) %>%
  mutate(n_cps = sum(person_weight_2/(10000*1000))) %>%
  ungroup() %>%
  distinct(year, month, sector, .keep_all = TRUE) %>%
  group_by(year, sector) %>%
  mutate(n_cps = mean(n_cps)) %>%
  ungroup() %>%
  distinct(year, sector, .keep_all = TRUE) %>%
  select(year, sector, n_cps) %>%
  arrange(year, sector)
####
...
temp_cps <- cps %>%
  ...
  #filter(month == mn & ( (year == yr & inter_num==6) | (year == yr-1 & inter_num==2) ))
a <- dim(temp_cps)[1]
temp_cps <- temp_cps %>%
  group_by(H_ID, P_ID) %>%
...
base <- base %>%
  ...
  mutate(diag_correct = ifelse(diag == 1 & n <= 10, 1, 0)) %>%
  select(-n, -diag)

temp <- base %>%
  filter(sector_dest != sector_ori) %>%
  group_by(year, H_ID, P_ID, sector_ori, sector_dest) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  mutate(sector_ori = ifelse(n >= 2, sector_dest, sector_ori)) %>%
  select(-n)
...
base <- base %>%
  ...
  #mutate(n_cps = sum(person_weight_2/(10000))) %>%
  ungroup() %>%
  distinct(year, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
  select(year, state_ori, sector_ori, state_dest, sector_dest, n_cps, diag_correct)


####borrar

# base <- base %>%
#   select(-H_ID, -P_ID, -gender) %>%
#   filter(person_weight_1!= -1) %>%
#   #filter(person_weight_2!= -1) %>%
#   group_by(year, inter_num, state_ori, sector_ori, state_dest, sector_dest) %>%
#   mutate(n_cps = sum(person_weight_1/(10000))) %>%
#   #mutate(n_cps = sum(person_weight_2/(10000))) %>%
#   ungroup() %>%
#   distinct(year, inter_num, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
#   group_by(year, state_ori, sector_ori, state_dest, sector_dest) %>%
#   mutate(n_cps = mean(n_cps)) %>%
#   ungroup() %>%
#   distinct(year, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
...
temp <- acs %>%
  ...
  mutate(sector_ori = sec_ori, n_cps = 0, diag_correct = 0)  
add <- rbind(add, temp)
}
...
write.table(cps, file = paste("1-Intermediate_Processed_Data//cps_temp.csv", sep=""), sep = ",", row.names = FALSE)

cps <- read.csv(paste("1-Intermediate_Processed_Data//cps_temp.csv", sep=""), header = TRUE, sep = ",", dec = ".")


####borrar
hola_2 <- cps %>%
  group_by(year, sector_dest) %>%
  mutate(n_cps = sum(n_cps)) %>%
  ungroup() %>%
  filter(year==2002 & sector_dest!=0) %>%
  distinct(year, sector_dest, .keep_all = TRUE) %>%
  select(year, sector_dest, n_cps) %>%
  arrange(year, sector_dest)
####
...
#   arrange(state, sector) %>%
#   dplyr::rename(denom = employ)

# #replacing 0 in diag
# cps <- cps %>%
#   left_join(L_2000, by=c("state_dest"="state", "sector_dest"="sector")) %>%
#   mutate(numer = ifelse(sector_ori == sector_dest & state_ori == state_dest, n_cps, 0)) %>%
#   group_by(year, state_dest, sector_dest) %>%
#   mutate(numer = max(numer)) %>%
#   ungroup() %>%
#   mutate(frac = ifelse(numer==0, NA, numer/denom)) %>%
#   #mutate(frac = ifelse(numer==0 | numer<10, NA, numer/denom)) %>%
#   mutate(frac = ifelse(sector_ori == sector_dest & state_ori == state_dest, frac, NA)) %>%
#   group_by(year, state_dest) %>%
#   mutate(frac = mean(frac, na.rm = TRUE)) %>%
#   ungroup() %>%
#   mutate(n_cps=ifelse(sector_ori==sector_dest & state_ori==state_dest & n_cps==0,denom*frac,n_cps)) %>%
#   #mutate(n_cps=ifelse(sector_ori==sector_dest & state_ori==state_dest & (n_cps==0 | n_cps<10),denom*frac,n_cps)) %>%
#   select(-numer, -denom, -frac)
...
cps <- cps %>%
  ...
  mutate(diag = ifelse(sector_ori == sector_dest & state_ori == state_dest, 1, 0)) %>%
  mutate(diag_correct = ifelse(diag == 1 & n_cps == 0, 1, diag_correct)) %>%
...
cps <- cps %>%
  ...
  select(-n_cps) %>%
  group_by(year, state_ori, sector_dest, sector_ori) %>%
...
# cps <- cps %>%
#   group_by(year, sector_dest, sector_ori) %>%
#   mutate(n_cps_national = sum(n_cps)) %>%
#   ungroup() %>%
#   mutate(n_cps = ifelse(state_dest != state_ori, n_cps_national, n_cps)) %>%
#   select(-n_cps_national)
...
# cps <- cps %>%
#   mutate(n_cps_temp = ifelse(state_ori==state_dest, n_cps, 0)) %>%
#   group_by(year, state_ori, sector_ori, sector_dest) %>%
#   mutate(n_cps_temp = max(n_cps_temp), mean_n_cps = mean(n_cps)) %>%
#   ungroup() %>%
#   mutate(n_cps = ifelse(n_cps_temp<n_cps, mean_n_cps, n_cps)) %>%
#   select(-n_cps_temp, -mean_n_cps)
# 
# cps <- cps %>%
#   mutate(n_cps_temp = ifelse(state_ori==state_dest, n_cps, 0)) %>%
#   group_by(year, state_dest, sector_ori, sector_dest) %>%
#   mutate(n_cps_temp = max(n_cps_temp), mean_n_cps = mean(n_cps)) %>%
#   ungroup() %>%
#   mutate(n_cps = ifelse(n_cps_temp<n_cps, mean_n_cps, n_cps)) %>%
#   select(-n_cps_temp, -mean_n_cps)
...
CPS_PUMS$year <- yr
cps_supp <- rbind(cps_supp, CPS_PUMS)
}
rm(CPS_PUMS) 
#creating variables
cps_household <- cps_supp %>%
  mutate(year = as.numeric(year)) %>%
...
cps_persons <- cps_supp %>%
  mutate(year = as.numeric(year)) %>%
  mutate(year = ifelse( (year!=99 & year!=98), 2000 + year-1, 1900+year-1)) %>%
  mutate(len = nchar(V1)) %>% #len matches data dictionary
  mutate(type = as.numeric(substr(V1, start=1, stop=1))) %>% 
  filter(type == 3) %>% # only persons  
  mutate(person_weight = as.numeric(substr(V1, start=66, stop=73))) %>% 
  mutate(ID_H = substr(V1, start=2, stop=6)) %>% #Household ID
  ...
  filter( is.na(employ_status) == FALSE &((employ_status>=1 & employ_status<=6)|employ_status==7)) %>%
  mutate(employ_status = as.numeric( recode( as.character(employ_status), '5' ="0", '6' ="0", '7' ="0", '4' ="0", '3' ="0", '2' = "1", '1' = "1"))) %>%
  ...
  mutate(census_sec = ifelse(employ_status == 0, NA, census_sec)) %>%
  mutate(census_sec = gsub(" ", "", census_sec)) %>%
  select(year, state_dest, sector_dest, state_ori, census_sec, person_weight)
#sectors 
cps_1 <- cps_persons %>%
  filter(year < 2003) %>%
  left_join(sectors_census1990_final, by=c('census_sec'='census_1990')) %>% #census_sec to final sector
...
base <- base %>%
  group_by(year, state_ori, sector_ori, state_dest, sector_dest) %>%
  mutate(n_cps = sum(person_weight/100)) %>%
  ungroup() %>%
  distinct(year, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
...
cps_supp <- cps_supp %>%
  left_join(temp, by=c("year"="year")) %>%
  mutate(diag = ifelse(sector_ori == sector_dest & state_ori == state_dest, 1, 0)) %>%
  mutate(diag_correct = ifelse(diag == 1 & n_cps == 0 , 1, 0)) %>%
  mutate(n_cps = ifelse(diag == 1 & n_cps == 0 , min_diag, n_cps)) %>%
  select(-min_diag, -diag)
rm(temp)

write.table(cps_supp, file = paste("1-Intermediate_Processed_Data//cps_supp.csv", sep=""), sep = ",", row.names = FALSE)

```

```{r}
cps <- read.csv(paste("1-Intermediate_Processed_Data//cps_2.csv", sep=""), header = TRUE, sep = ",", dec = ".")
#cps <- read.csv(paste("1-Intermediate_Processed_Data//cps_supp.csv", sep=""), header = TRUE, sep = ",", dec = ".")

# cps <- rbind(cps, cps_supp)
# cps <- cps %>%
#   group_by(year, state_ori, sector_ori, state_dest, sector_dest) %>%
#   mutate(n_cps = sum(n_cps)) %>%
#   ungroup() %>%
...
cps <- cps %>%
  mutate(diag_correct = ifelse(state_ori=="Alaska" & state_dest=="Alaska" & sector_ori==10 & sector_dest==10, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="Alaska" & state_dest=="Alaska" & sector_ori==8 & sector_dest==8, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="Montana" & state_dest=="Montana" & sector_ori==6 & sector_dest==6, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="Maine" & state_dest=="Maine" & sector_ori==6 & sector_dest==6, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="Idaho" & state_dest=="Idaho" & sector_ori==9 & sector_dest==9, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="Alaska" & state_dest=="Alaska" & sector_ori==12 & sector_dest==12, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="NorthDakota" & state_dest=="NorthDakota" & sector_ori==11 & sector_dest==11, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="NewJersey" & state_dest=="NewJersey" & sector_ori==3 & sector_dest==3, 1, diag_correct)) %>%
  mutate(diag_correct = ifelse(state_ori=="NewJersey" & state_dest=="NewJersey" & sector_ori==14 & sector_dest==14, 1, diag_correct)) 

######Small num observations fix

cps <- cps %>%
  mutate(n_cps_temp = ifelse(state_ori==state_dest, n_cps, 0)) %>%
  group_by(year, state_ori, sector_ori) %>%
  mutate(tot_within_state_ori = sum(n_cps_temp)) %>%
  mutate(tot_ori = sum(n_cps), diag_correct_ori = max(diag_correct)) %>%
  ungroup() %>%
  mutate(n_cps = ifelse(diag_correct_ori == 1 & state_ori==state_dest & sector_ori==sector_dest, tot_ori, n_cps)) %>%
  mutate(n_cps = ifelse(diag_correct_ori == 1 & (state_ori!=state_dest | sector_ori!=sector_dest), 0, n_cps)) %>%
  group_by(year, state_dest, sector_dest) %>%
  mutate(tot_within_state_dest = sum(n_cps_temp)) %>%
  mutate(tot_dest = sum(n_cps), diag_correct_dest = max(diag_correct)) %>%
  ungroup() %>%
  mutate(n_cps = ifelse(diag_correct_dest == 1 & state_ori==state_dest & sector_ori==sector_dest, tot_dest, n_cps)) %>%
  mutate(n_cps = ifelse(diag_correct_dest == 1 & (state_ori!=state_dest | sector_ori!=sector_dest), 0, n_cps)) %>%
  #mutate(n_cps = ifelse(tot_within_state < 650 & state_ori==state_dest & sector_ori==sector_dest, tot, n_cps)) %>%
  #mutate(n_cps = ifelse(tot_within_state < 650 & (state_ori!=state_dest | sector_ori!=sector_dest), 0, n_cps)) %>%
  select(year, state_dest, sector_dest, state_ori, sector_ori, n_cps)
...
temp <- cps %>%
  filter(year<=2000)
write.table(temp, file = paste("2-Final_Data//cps_1999_2000.csv", sep=""), sep = ",", row.names = FALSE)
temp <- acs %>%
  filter(year<=2000)
write.table(temp, file = paste("2-Final_Data//acs_1999_2000.csv", sep=""), sep = ",", row.names = FALSE)
temp <- mobility %>%
  filter(year<=2000)
write.table(temp, file = paste("2-Final_Data//x_adj_1999_2000.csv", sep=""), sep = ",", row.names = FALSE)
...
temp_USA <- SEA_1 %>%
  filter(country == "USA" & (year == 1999 | year == 2000)) %>%
  arrange(year, sector) %>%
  mutate(year = paste("y", year , sep=""))
temp_USA <- spread(temp_USA, year, employ, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
temp_USA <- temp_USA %>%
  mutate(flow = y2000-y1999) %>%
  mutate(flow_perc = (flow*100/y1999))
...
temp_USA_prop <- SEA_1 %>%
  ...
  arrange(year, sector) %>%
  mutate(year = paste("y", year , sep=""))
temp_USA_prop <- spread(temp_USA_prop, year, employ, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
temp_USA_prop <- temp_USA_prop %>%
  mutate(flow = y2000-y1999) %>%
  mutate(flow_perc = (flow*100/y1999))
...
```{r}
#mu_1999 <- mu_all %>% filter(year == 1999)
#Recalculating mu_1999 and checking flow 1999-2000
mu_1999 <- mu_1999_prime %>% 
  dplyr::rename(n_cps_adj = level_employ) %>%
  group_by(year, state_ori, sector_ori) %>% 
...
t <- acs %>% 
  filter(year==1999 & state_dest=="Alaska" & state_ori!="Alaska")
print(sum(t$n_acs))
t <- acs %>% 
  filter(year==1999 & state_dest!="Alaska" & state_ori=="Alaska")
print(sum(t$n_acs))
