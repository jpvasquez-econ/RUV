...
# base <- base %>%
#   select(-H_ID, -P_ID, -gender, -inter_num) %>%
#   #filter(person_weight_1!= -1) %>%
#   filter(person_weight_2!= -1) %>%
#   group_by(year, month, state_ori, sector_ori, state_dest, sector_dest) %>%
#   #mutate(n_cps = sum(person_weight_1/(10000))) %>%
#   mutate(n_cps = sum(person_weight_2/(10000))) %>%
#   ungroup() %>%
#   distinct(year, month, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
#   group_by(year, state_ori, sector_ori, state_dest, sector_dest) %>%
#   mutate(n_cps = mean(n_cps)) %>%
#   ungroup() %>%
#   distinct(year, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
...
base <- base %>%
  group_by(year, state_ori, sector_ori, state_dest, sector_dest) %>%
  mutate(n_cps = sum(person_weight)) %>%
  ungroup() %>%
  distinct(year, state_ori, sector_ori, state_dest, sector_dest, .keep_all = TRUE) %>%
...
#cps_supp <- read.csv(paste("1-Intermediate_Processed_Data//cps_supp.csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
#   mutate(tot_within_state = sum(n_cps_temp)) %>%
...
#   select(-tot, -tot_within_state, -n_cps_temp)
