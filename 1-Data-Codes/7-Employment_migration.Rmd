...
#Some check and adjustment (all based on three-year window)
...
#cps_2.csv: all migration combinations by (state_ori, state_dest, sector_ori, sector-dest) for 1999-2006; n_cps contains within destination state migration across sectors for each migration combination, repeating within within (year, state_dest, sector_dest)
...
mobility <- cps %>%
  ...
  mutate(tot_n_cps_accros_sec = sum(n_cps)) %>% #totol within destination state migration
  ungroup() %>%
...
mu_1999_prime <- mobility %>%
  filter(year == 1999) %>%
  group_by(year, state_dest, sector_dest) %>% 
  mutate(tot_n = sum(n_cps_adj)) %>% #total number of people at destination state-sector
  ungroup() %>%
  mutate(rel = n_cps_adj/tot_n) %>%  #rel: share of people from original state-sector pairs to the same destination state-sector
...
mobility <- mobility %>% 
  ...
  mutate(tot_n_cps_accros_dest = sum(n_cps_adj)) %>% #total number of people at original state-sector pair
  ungroup() %>%
  mutate(rel = n_cps_adj/tot_n_cps_accros_dest) %>% #rel: share of people moved out from the same original state-sector pair to different destination state-sector pair 
...
temp <- final %>%
  ...
  filter(diag == 1 & rel != 0) %>%
  group_by(diag) %>%
  mutate(min_diag = min(rel)) %>% #minimum share same year within state migration
  ungroup()
a <- temp$min_diag[1] #the share of within state migration (among migration flow share out of the state-sector) is the minimum among all US states in this year
final <- final %>%
  mutate(diag = ifelse(sector_ori == sector_dest & state_ori == state_dest, 1, 0)) %>%
