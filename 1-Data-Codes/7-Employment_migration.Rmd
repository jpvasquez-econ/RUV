...
mu_base <- read.csv(paste("1-Intermediate_Processed_Data/mu_", yr-1,".csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop(paste("mobility shares do not add to 1,", yr))
write.table(mu_1999, file = paste("1-Intermediate_Processed_Data//mu_1999.csv", sep=""), sep = ",", row.names = FALSE)
...
write.table(L, file = paste("1-Intermediate_Processed_Data/L_", yr,".csv", sep=""), sep = ",", row.names = FALSE)
