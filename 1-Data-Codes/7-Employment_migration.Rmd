...
census <- census %>%
  ...
  #mutate(weight = as.numeric(substr(V1, start=13, stop=16))) %>%
...
base <- base %>%
  mutate(diag_correct = 0)

# temp <- base %>%
#   distinct(year, H_ID, P_ID, .keep_all=TRUE) %>%
#   select(year, H_ID, P_ID, sector_dest, sector_ori)
# base <- base %>%
#   select(-sector_dest, -sector_ori) %>%
#   left_join(temp, by=c("year"="year", "H_ID"="H_ID", "P_ID"="P_ID"))
  
# temp <- base %>%
#   filter(sector_dest != sector_ori) %>%
#   distinct(year, H_ID, P_ID, sector_ori, sector_dest, .keep_all = TRUE)
# 
# base <- base %>%
#   filter(sector_dest == sector_ori)
# base <- rbind(base, temp)
...
#   mutate(max_row = max(n_cps)) %>%
#   mutate(sum_row = sum(n_cps)) %>%
#   ungroup() %>%
#   mutate(diag_correct = ifelse(diag==1 & n_cps!=max_row, 1, diag_correct)) %>%
#   mutate(diag_correct = ifelse(diag==1 & (n_cps/sum_row)<0.7, 1, diag_correct)) %>%
#   mutate(diag_correct = ifelse(is.na(diag_correct)==TRUE, 1, diag_correct)) %>%
#   select(-diag, -max_row, -sum_row)
##
...
#   select(year, state_dest, sector_dest, state_ori, sector_ori, n_cps, diag_correct)
######
...
# correction <- read.csv(paste("1-Intermediate_Processed_Data//correction.csv", sep=""), header = TRUE, sep = ",", dec = ".")
# colnames(correction) <- c("region", "sector")
# for (i in 1:(dim(correction)[1]) ) {
# st <- correction$region[i]
# sec <- correction$sector[i]
# cps <- cps %>%
#   mutate(diag_correct = ifelse(state_ori==st & state_dest==st & sector_ori==sec & sector_dest==sec, 1, diag_correct))
# }
