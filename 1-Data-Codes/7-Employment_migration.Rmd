...
mu_1999_levels <- mobility %>%
  filter(year == 1999) %>%
  mutate(n_cps_adj = ifelse(n_cps_adj==0 & state_dest==state_ori & sector_dest== sector_ori, 1, n_cps_adj)) %>%
...
write.table(mu_1999_levels, file = paste("1-Intermediate_Processed_Data/mu_1999_levels.csv", sep=""), sep = ",", row.names = FALSE)
...
write.table(final, file = paste("2-Final_Data//mu_", yr, ".csv", sep=""), sep = ",", row.names = FALSE)
}

#write.table(mu_all, file = paste("1-Intermediate_Processed_Data//mu_all.csv", sep=""), sep = ",", row.names = FALSE)

rm(acs, cps, mobility, mu_1999_levels, check, final, mu_all, temp)

#First attempt to compute L_1999 gives negatives in L
...
#mu_1999 levels for L_1999
mu_1999_levels <- read.csv(paste("1-Intermediate_Processed_Data/mu_1999_levels.csv", sep=""), header = TRUE, sep = ",", dec = ".")
L_2000 <- read.csv(paste("1-Intermediate_Processed_Data/L_2000.csv", sep=""), header = TRUE, sep = ",", dec = ".")
L_2000 <- melt(L_2000, id.vars=c("state"), variable.name="sector", value.name = "employ")
L_2000 <- L_2000 %>% 
  mutate(sector = as.numeric(gsub("X", "", sector)))
mu_1999_levels <- mu_1999_levels %>%
...
rm(mu_1999_levels, L_2000, SEA_US_2000, SEA_1)
```
...
mu_base <- read.csv(paste("2-Final_Data/mu_", yr-1,".csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop(paste("mobility shares do not add to 1,", yr))
write.table(mu_1999, file = paste("2-Final_Data//mu_1999.csv", sep=""), sep = ",", row.names = FALSE)
...
write.table(L, file = paste("2-Final_Data/L_", yr,".csv", sep=""), sep = ",", row.names = FALSE)
