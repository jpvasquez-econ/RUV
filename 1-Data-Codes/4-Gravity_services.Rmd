...
#	4. Proportionality with WIOD (expenditure by states sums up to US imports; revenue by states sums up to US exports).
bea_raw <- bea_raw %>%
...
wiod <- wiod_base %>% 
  ...
  filter( importer != 'USA' ) %>% 
  select( -year, -USA, -sector, -importer  ) 
## Gravity dataset (distances, revenue, and expenditure)
