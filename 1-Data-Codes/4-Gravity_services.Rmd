...
1) Constructs Revenue and Expenditures services data by state.
...
6. `1-Intermediate_Processed_Data/state_exp_rev_services.csv`
...
# Revenue and Expenditures services data
...
#	1. Constructs the data of production by state
...
## Merging with expenditure and revenue at the country level

```{r}

# remember the formula for distances is $d_{ij} = (\sum_{r \in i} \sum_{s \in j} (\dfrac{pop_r}{pop_i}) (\dfrac{pop_s}{pop_j}) d_{rs}^\theta)^{1/ \theta}$
...
# Merging EXPENDITURE AND REVENUE
final <- distances %>%
...
# This function solves the equilibrum system to construct the data for RUV using a gravity approach 
