...
#WIOD
WIOD <- read_dta("1-Intermediate_Processed_Data\\WIOD_countries.dta")
...
wiod <- WIOD %>% filter(importer_country!= "USA")
countries=unique(wiod$importer_country)
...
rownames(city.count)=c(1:dim(matrix(countries)))
city.count=data.frame(city.count)
city.count$country=countries
...
countries_states= c("USA", countries)
for (i in 1:dim(UN.1)[1]) {
