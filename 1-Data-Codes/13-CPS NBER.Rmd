...
base  <- base  %>% 
  filter(n_sample >= num_sample)  # 224 observations dropped
...
#### Case 3: Solution 1 from lyx 3.3.1 on 27th Oct #### 
# loading intermediate CPS
cps <- read.csv(paste("1-Intermediate_Processed_Data/cps_nber_2_yearly.csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
temp_between_acs  <-  acs  %>%  # create the acs share for between state expression
  ...
  ungroup()  %>% 
  group_by(year, state_dest, sector_dest)  %>% 
  mutate(n_acs_dest = sum(n_acs))  %>% 
  ungroup()  %>% 
  mutate(acs_share = n_acs_dest/n_acs_two_tot)  %>% 
  mutate(acs_share = ifelse(is.na(acs_share), 0, acs_share))  %>% 
  select(year, state_ori, state_dest, sector_dest, acs_share)
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
write.table(mu_1999_prime, file = paste("2-Final_Data/mu_1999_nber_case3_yearly.xlsx", sep=""), sep = ",", row.names = FALSE)
...
#### Case 4: Solution 2 from lyx 3.3.1 on 27th Oct ####
# loading intermediate CPS
cps <- read.csv(paste("1-Intermediate_Processed_Data/cps_nber_2_yearly.csv", sep=""), header = TRUE, sep = ",", dec = ".")
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
write.table(mu_1999_prime, file = paste("2-Final_Data/mu_1999_nber_case4_yearly.xlsx", sep=""), sep = ",", row.names = FALSE)
...
cps_nber  <- cps_nber  %>% 
  group_by(P_ID)  %>% 
  mutate(indicator = ifelse(any(inter_num  %in%  c(1)), 1, 0))  %>% 
  ungroup()

mn_list <- c("jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec")
quart_inter  <- c(1, 4, 5, 8)

# choose the number of observations within each household
num_obs  <- c(1)  # c(1): only keep period 1 and 4; c(1, 2): keep period 1 and 4 as well as period 5 and 8

# choose the restrictions on the sample size
num_sample  <- 10

#cps_nber <- cps_nber %>%.  # This is to keep HH with 1-8 complete first
#  group_by(P_ID) %>%
#  filter(all(c(1,2,3,4,5,6,7,8)  %in% inter_num))  %>%       
#  ungroup()

cps_nber  <- cps_nber  %>% 
  filter(inter_num %in% quart_inter)  %>% 
  mutate(group = ifelse(inter_num %in% c(1,4), 1, 2))

cps_nber  <- cps_nber  %>% 
  group_by(P_ID, group)  %>% 
  filter(
    (all(c(1, 4) %in% inter_num)) |    # keep observation when 1 and 4 both appear
    (all(c(5, 8) %in% inter_num))      # keep observation when 5 and 8 both appear
  )  %>% 
  ungroup() 
  
cps_nber  <- cps_nber  %>% 
  filter(group %in% num_obs)   # keep one or two pair of observations for each HH
...
temp <- full_join(info_present, info_past, by = c("P_ID"))  %>% # after matching, get state info for t and t-1 from CPS
  filter( inter_num.x - inter_num.y == 3 )
...
base  <- base  %>% 
  filter(n_sample >= num_sample)  # 1450 observations dropped
#base  <- base  %>% 
#  filter(n_sample >= 5)  
...
### Quarterly matrix with IRS, ACS and CPS (Case 4)
...
mu_1999_prime_q1 <- mu_1999_prime_q1 %>%
  mutate(sector_dest = recode(as.character(sector_dest), '0' ="00", '1' ="01", '2' ="02", '3' ="03", '4' ="04", '5' ="05", '6' ="06", '7' ="07", '8' ="08", '9' ="09" )) %>%
  mutate(state_dest_sector_dest = paste(state_dest, sector_dest, sep="_")) %>%
  select(-state_dest, -sector_dest, -quarter)

mu_1999_prime_q1 <- spread(mu_1999_prime_q1, state_dest_sector_dest, rel, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
A  <- mu_1999_prime_q1[,4:ncol(mu_1999_prime_q1)]
# check sum(prob)==1
epsilon <- 0.01
check <- mu_1999_prime_q1 %>% select(-year, -state_ori, -sector_ori)
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
...
mu_1999_prime_q2 <- mu_1999_prime_q2 %>%
  ...
  mutate(state_dest_sector_dest = paste(state_dest, sector_dest, sep="_")) %>%
  select(-state_dest, -sector_dest, -quarter)
mu_1999_prime_q2 <- spread(mu_1999_prime_q2, state_dest_sector_dest, rel, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
B  <- mu_1999_prime_q2[,4:ncol(mu_1999_prime_q2)]
# check sum(prob)==1
epsilon <- 0.01
check <- mu_1999_prime_q2 %>% select(-year, -state_ori, -sector_ori)
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
...
mu_1999_prime_q3 <- mu_1999_prime_q3 %>%
  ...
  mutate(state_dest_sector_dest = paste(state_dest, sector_dest, sep="_")) %>%
  select(-state_dest, -sector_dest, -quarter)
mu_1999_prime_q3 <- spread(mu_1999_prime_q3, state_dest_sector_dest, rel, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
C  <- mu_1999_prime_q3[,4:ncol(mu_1999_prime_q3)]
# check sum(prob)==1
epsilon <- 0.01
check <- mu_1999_prime_q3 %>% select(-year, -state_ori, -sector_ori)
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
...
mu_1999_prime_q4 <- mu_1999_prime_q4 %>%
  ...
  mutate(state_dest_sector_dest = paste(state_dest, sector_dest, sep="_")) %>%
  select(-state_dest, -sector_dest, -quarter)
mu_1999_prime_q4 <- spread(mu_1999_prime_q4, state_dest_sector_dest, rel, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)
D  <- mu_1999_prime_q4[,4:ncol(mu_1999_prime_q4)]
# check sum(prob)==1
epsilon <- 0.01
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
write_xlsx(mu_1999_annualise, path = paste("1-Intermediate_Processed_Data/mu_1999_nber_case4_qr_annualised.xlsx", sep=""))

```


### Quarterly matrix with IRS, ACS and CPS (Case 3)
...
temp_between_acs  <-  acs  %>%  # create the acs share for between state expression
  filter(year == 1999)  %>% 
  group_by(year, quarter, state_dest)  %>% 
  mutate(n_acs_two_tot = sum(n_acs))  %>% 
  ungroup()  %>% 
  group_by(year, quarter, state_dest, sector_dest)  %>% 
  mutate(n_acs_dest = sum(n_acs))  %>% 
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop("mobility shares do not add to 1")
write_xlsx(mu_1999_annualise, path = paste("1-Intermediate_Processed_Data/mu_1999_nber_case3_qr_annualised.xlsx", sep=""))
