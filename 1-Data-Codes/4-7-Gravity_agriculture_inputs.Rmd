...
wiot.full <- wiot.full %>% 
  filter(year>=2000, year<=2007, col_country!="LUX", col_country!="LVA", col_country!="MLT", row_country!="LUX", row_country!="LVA", row_country!="MLT") %>% #since distances are available from that year on
...
n.c <- 2;
n.s <- 2;
# WIOD.base <- as.data.frame(matrix(0, nrow = 3, ncol = 3))
# colnames(WIOD.base) <- c("value_USA", "value_AUS", "value_AUT")
# WIOD.base <- WIOD.base %>%
#   mutate(sector=14) %>%
#   mutate(year=2000) %>%
#   mutate(importer_country=c("USA", "AUS", "AUT")) %>%
#   mutate(importer=c(1,2,3)) %>%
#   select(year, importer_country, importer, sector, everything())
...
#   mutate(X_j= ifelse(iso_d=="Alabama", example$X_j[3], X_j)) %>%
#   mutate(X_j= ifelse(iso_d=="Alaska",  example$X_j[4], X_j)) %>%
...
# ######################################## checks: Y
# c.names <- as.vector(unique(WIOD.base$importer_country))
# c.names <- c.names[2:38]
# #checks for sectors
# for (s in 1:12) {
#   h <- WIOD.base %>% filter(year==yr, sector==s)
#   h1=sum(h[(1:38), 5])
#   HOLIS <- state.exports %>%
#     filter(year==yr, sector==s) %>%
#     select(-year, -importer, -sector)
#   h2=sum(HOLIS)
#   print(h1-h2)
# }
...
# 
# 
# #services.Y.countries (RUS and RoW order)
# h <- data_gravity_serv.base %>%
#   filter(year==yr, iso_o %in% c.names) %>%
#   distinct(iso_o, .keep_all = TRUE)
# h2=0
# for (i in c.names) {
# h <- WIOD.base %>% filter(year==yr, sector==13)
# print(sum(h[(1:38), 6+h2]))
# h2=h2+1
# }
# 
# #services.Y.states ()
# h <- data_gravity_serv.base %>%
#   filter(year==yr, iso_o %in% state.names) %>%
#   distinct(iso_o, .keep_all = TRUE)
# h1=sum(h$Y_i)
# h <- WIOD.base %>% filter(year==yr, sector==13)
# h2=sum(h[(1:38), 5])
...
#####################################################
##
##CALCULATING \sum_{s}\phi_{14,s}Y_{j,s} (SUM OF INTERMEDIATE USES IN AGRIC) FOR EACH STATE
##
#reshaping gdp.tot.v
...
labor.shares <- read.csv( file= paste("1-Intermediate_Processed_Data//labor_shares_countries", yr, ".csv", sep=""))%>%
             filter(region=="USA") %>%
             select(-year, -region);
labor.shares <- matrix( data=rep(as.matrix(labor.shares), (14)), nrow=(14), byrow=TRUE );
...
#check: sum(X_j)=WIOD
check1 <- WIOD.base %>%
  filter(year==yr, importer_country=="USA", sector==14)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
## Calculating lambda and B matrix
...
print(max(lambda_vector))
print(min(lambda_vector))
