...
##
#reshaping gdp.tot.v
gdp.tot.v.mat<-spread(gdp.tot.v, exporter, export, fill = NA, convert = FALSE, drop = TRUE, sep = NULL) %>%
               select(-sector);
#product of a 1x14 row vect with \phi_{14,s} and a 14x50 matrix with the Y_j,s
sum.14<-as.matrix(io.0)  %*% as.matrix(gdp.tot.v.mat);
sum.14<-t(sum.14.v); #i confirmed this is the same as the other calculation
...
expenditure.mat<-spread(expenditure.vector.serv.s, importer, import, fill = NA, convert = FALSE, drop = TRUE, sep =                  NULL) %>%  select(-r);
...
sum.not.14.v<-as.matrix(io.shares)  %*% as.matrix(gdp.tot.v.mat);
sum.not.14.v<-as.data.frame(t(sum.not.14.v)) %>%
            select(-V14);
#sum over r
sum.not.14.vector<-expenditure.mat-sum.not.14.v
sum.not.14.vector<-apply(as.data.frame(sum.not.14.vector),MARGIN=1, FUN=sum)
...
names(exp.v)<-c("export","sector","exporter")
gdp.tot.v <- rbind(exp.v, gdp.vector.serv.s, gdp.vector.agri.s)
gdp.tot.v$phi <-NA
for (sec in 1:14) {
  gdp.tot.v <- gdp.tot.v %>%
...
#############################
############################# MY WAY - MARCO'S WAY are different
#############################
as.vector(sum.not.14.vector) -sum.not.14$sum.not.14
