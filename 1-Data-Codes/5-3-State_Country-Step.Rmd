...
To use $X_{ij,k}^{Base}$ and $X_{ij,k}^{WIOD}$ plus proportionality to calculate $X_{ij,k}^{year}$ for $i\in US\;  \&\;  j \notin US$ (exports from each State to each country) and $i\notin US\;  \&\;  j \in US$ (imports from each country to each State), and $\forall k=1,...,12,14$.  I call the output of this part $X_{ij,k}^{Step3}$. $Base=2002$ for exports and $Base=2008$ for imports. 
...
\begin{enumerate}
\item For $j\neq US$ we should $\sum_{i\in US}X_{ij,k}^{step3}=X_{US\,j,k}^{WIOD} \; \forall j, k=1,...,12, 14$
\item For $i\notin US$ we should $\sum_{j\in US}X_{ij,k}^{step3}=X_{i\,US,k}^{WIOD} \; \forall i,k=1,...,12, 14$
\end{enumerate}
