...
libs <- c( 'dplyr', 'tidyr', 'readstata13', 'haven', 'stringr', 'nleqslv', 'gdata', 'Statamarkdown', 'formattable', 'ggplot2', 'viridis', 'RColorBrewer', 'reshape2', 'foreign', 'readxl', 'writexl', 'rjson', 'read_excel', 'Matrix', 'geodist', 'data.table');
...
2) This code computes the mobility matrices for each state and sector for years 1999-2006. 
...
#sector-country employment
SEA_1 <- SEA %>%
  filter(variable == "EMP") %>%
  select(-variable, -"2010", -"2011", -code)
SEA_1 <- reshape2::melt(SEA_1, id.vars=c("country", "description"), variable.name="year", value.name = "employ")
...
SEA_2 <- SEA_1 %>%
  ...
  distinct(year, region, sector, .keep_all = TRUE) %>%
  group_by(year, region) %>%
  mutate(tot_employ = sum(employ)) %>%
  ungroup() %>%
  select(-country,-tot_employ)
...
#merge with SEA
SEA_2 <- SEA_2 %>%
  left_join(bilat, by=c("region"="exporter", "sector"="sector")) %>%
  left_join(bilat_0, by=c("region"="exporter")) %>%
  mutate(R = ifelse(sector == 0, R_0, R)) %>% 
  mutate(prod_by_employ = ifelse(region != "RoW", R/employ, NA)) %>%
  group_by(year, sector) %>%
  mutate(avg_prod_by_employ = mean(prod_by_employ, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(employ = ifelse(region == "RoW", R/avg_prod_by_employ, employ)) %>%
  select(-R, -R_0, -prod_by_employ, -avg_prod_by_employ)

###############################################################################
#Sectors must be weighted to be consistent with our new sector 0 and RoW lfp rate according to World Bank.
SEA_countries <- read.csv("1-Intermediate_Processed_Data/SEA_countries.csv")
total_2000_RoW <- sum(subset(SEA_countries, year == 2000 & country == 'RoW')$employ)
participation_rate <- read.csv("0-Raw_Data/LFP_WB.csv")
oldlfp_RoW <- subset(participation_rate, Country.Code == 'WLD')$X2000
oldlfp_RoW <- oldlfp_RoW/100
newlfp_RoW <- 1 - (subset(SEA_countries, year == 2000 & country == 'RoW' & sector == 0)$employ)/total_2000_RoW
weight <- (oldlfp_RoW/(1-oldlfp_RoW))*((1-newlfp_RoW)/newlfp_RoW)
SEA_2$employ <- ifelse(SEA_2$region == 'RoW' & SEA_2$year == 2000, SEA_2$employ * weight, SEA_2$employ)
###############################################################################


#saving
write.table(SEA_2, file = paste("1-Intermediate_Processed_Data//L_1999_2000_countries.csv", sep=""), sep = ",", row.names = FALSE)

#US sector-country employment (WE ARE CHANGING THIS PART TO USE BLS)
# load CPS data from NBER
cps_nber  <- haven::read_dta("0-Raw_Data/CPS/CPS_NBER/Outputs/clean_data_new.dta")
write.table(cps_nber, file = paste("1-Intermediate_Processed_Data/cps_nber_yearly.csv", sep=""), sep = ",", row.names = FALSE)

cps_nber <- read.csv("1-Intermediate_Processed_Data/cps_nber_yearly.csv")

cps_nber2 <- cps_nber %>%
  filter(year >= 1999 & year <= 2000) %>%
  group_by(year, st_name, sector) %>%
  mutate(employment = sum(wtfinl)) %>%
  ungroup() %>%
  distinct(year, st_name, sector, .keep_all = TRUE) %>%
  select(year, st_name, sector, employment)

US_total_2000 <- cps_nber2 %>%
  filter(year == 2000) %>%
  mutate(US_total = sum(employment)) %>%
  distinct(year, .keep_all = TRUE) %>%
  select(US_total) 
US_total_2000 <- as.numeric(US_total_2000/1000)


cps_nber2 <- cps_nber2 %>%
  group_by(year) %>%
  mutate(US_total = sum(employment)) %>%
  mutate(share_proportionality = US_total_2000/US_total) %>%
  mutate(employment = employment*share_proportionality) %>%
  ungroup() %>%
  select(employment, year, sector, st_name)
...
#here we want to apply proportionality so that things add up to US - WIOD sector by sector
...
L_2000 <- L_2000 %>%
  ...
  mutate(employ2 = (sum_SEA_US_2000_not0/sum_L_data_not0)*employ)
L_2000 <- spread(L_2000, sector, employ, fill = NA, convert = TRUE,  drop = TRUE, sep = NULL)

#saving
write.table(cps_nber2, file = paste("1-Intermediate_Processed_Data//L_1999_2000_states.csv", sep=""), sep = ",", row.names = FALSE)
...
# use up to 2 observations
#temp_cps_nber <- cps_nber %>%
#  filter(month == mn & ( (year == yr & inter_num==5) | (year == yr-1 & inter_num==1) | (year == yr & inter_num==6) | (year == yr-1 & inter_num==2))) 
# use up to 4 observations
#temp_cps_nber <- cps_nber %>%
#  filter(month == mn & ( (year == yr & inter_num  %in% c(5, 6, 7, 8)) | (year == yr-1 & inter_num  %in% c(1, 2, 3, 4)) ) ) 
...
CENSUS_PUMS <- CENSUS_PUMS %>%
  ...
  mutate(naics = substr(X1, start=215, stop=217)) %>%
...
for (st in st_list) {
#print(st)  
if (yr_c == 2000){ACS_PUMS <- read_csv(paste("0-Raw_Data/ACS/20", yr,"/c2ssp", st,".csv", sep=""), col_types = cols())}  
if (yr_c != 2000){ACS_PUMS <- read_csv(paste("0-Raw_Data/ACS/20", yr,"/ss", yr,"p", st,".csv", sep=""), col_types = cols())}    
ACS_PUMS$year <- yr_c
ACS_PUMS <- ACS_PUMS %>%
...
write_xlsx(final, paste("2-Final_Data//mu_", yr, "_nomigration.xlsx", sep=""))
}
...
sum_SEA_US_2000_not0 <- SEA_US_2000$sum_SEA_US_2000_not0[1]
L_2000 <- read.csv("1-Intermediate_Processed_Data/L_2000_base.csv", header = TRUE, sep = ",", dec = ".")
L_2000 <- reshape2::melt(L_2000, id.vars=c("state"), variable.name="sector", value.name = "employ")
L_2000 <- L_2000 %>% 
  mutate(sector = as.numeric(gsub("X", "", sector))) %>%
  arrange(state, sector) %>%
  mutate(employ_temp = ifelse(sector == 0, 0, employ)) %>%
...
mu_base <- read_excel(paste("2-Final_Data/mu_", yr-1,"_nomigration.xlsx", sep=""))
...
if(sum(abs(as.matrix(rowSums(check)) - matrix(1, dim(check)[1], 1))) > epsilon) 
  stop(paste("mobility shares do not add to 1,", yr))
write_xlsx(mu_1999, path = paste("2-Final_Data//mu_1999_nomigration.xlsx", sep=""))
...
write_xlsx(L, path = paste("2-Final_Data/L_", yr,"_nomigration.xlsx", sep=""))
}
...
L <- read_excel(paste("2-Final_Data/L_", yr,"_nomigration.xlsx", sep=""))
mu <- read_excel(paste("2-Final_Data/mu_", yr,"_nomigration.xlsx", sep=""))
...
write_xlsx(table, path = paste("2-Final_Data/table_I_", yr,"_nomigration.xlsx", sep=""))
}
