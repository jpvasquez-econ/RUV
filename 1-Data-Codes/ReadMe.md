...
$$\tilde{\tau}_{ij}=\exp(\hat{\beta}_{0}\iota_{ij}+\hat{\beta}_{1}\ln dist_{ij})$$
...
where $R_{i}$ is total revenue of region $i$; $E_{j}$ is total expenditure of region $j$; and $\tilde{\Pi}_{i}$ and $\tilde{P}_{j}$ are price indices that solve the gravity system (see scripts 4 and 5 for a detailed exposition of the system).
...
3) For script 8: `0-Raw_Data/CENSUS_2000/L_1999_2000_countries.csv`, `0-Raw_Data/CENSUS_2000/EAP_2WAP_SEX_AGE_RT_A-filtered-2023-11-02.csv`, and `0-Raw_Data/CENSUS_2000/P_World unemployment rates.xlsx` were not uploaded. However, we explain in the raw data ReadMe file how to download these files directly from WIOD's, ILO's and World Bank's pages, respectively.
4) For script 9: the collection of `0-Raw_Data/ACS/20*/c2ssp**.csv` .txt files for each US state was too heavy to be uploaded. However, we explain in the raw data ReadMe file how to download these files directly from  U.S. Census Bureau's page.
5) For script 9: `0-Raw_Data/CPS/CPS_NBER/Inputs/cps_panel.dta` was too heavy to be uploaded. However, we explain in the raw data ReadMe file how to download it.
