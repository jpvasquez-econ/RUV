...
1) **Task 1**: This code computes the employment level for each state and sector in year 2000 using CBS data.
2) **Task 2**: This code computes the employment level for each state and sector in year 2000 and year 1999 using BLS data.
3) **Task 3**:  This code computes the employment level for each country and sector in year 1999 and year 2000 using ILO and SEA data. 
4) **Task 4**: This code reshapes the outputs of step 1, step 2 and step 3, and binds them. It also applies a proportionality rule to step 1 and step 2 for consistency with SEA (and with zero population growth too in BLS' case). 
...
1. Construct table for sector classifications.
2. Construct list of US states.
...
1. Construct employment levels for each US state from Census 2000: distribute total employment between our 14 sectors using a proportionality rule.
...
for (i in 1:14) {
  sector <- paste0("sector_", i)
  assign(sector, select(get(sector), employment, area_title) %>%
           distinct()
  )
}
...
2. Append non-participants to SEA values to put together "employment" for all of our sectors for each country.

For any country, let $L_{tot}$ represent working age population, $L_{work}$ represent employment, $unemp$ represent unemployment, and $L_{i}$ represent "employment" in sector $i$ $\forall i=0,...,14$. If we define $L_0$ as non-participants in the labor force, and if we assume that the labor market only has 14 sectors, then we know that:
$$
L_{tot}	=L_{0}+unemp+L_{work}
$$
$$
L_{work}	=\sum_{i=1}^{14}L_{i}
$$
From ILO we know the value of the labor force participation rate ($lfp$) for each country; which using our assumptions satisfies
$$
1-lfp	=\dfrac{L_{0}^{ILO}}{L_{tot}^{ILO}}
$$
Superscripts simply indicate where the data is taken from; we adopt the same convention in what follows.

Also, from ILO we know the value of the unemployment rate ($u$) for each country, which is:
$$
u	=\dfrac{unemp^{ILO}}{unemp^{ILO}+L_{work}^{ILO}}
$$
For the SEA values of each of our 14 (employment) sectors to be consistent with $u$ and $lfp$, we must have
$$
1-lfp = \dfrac{L_{0}^{SEA}}{L_{0}^{SEA}+unemp^{SEA}+L_{work}^{SEA}} \; \; \; (1)
$$
$$
unemp^{SEA}	=u\times(L_{work}^{SEA}+unemp^{SEA}) \; \; \; (2)
$$
Equation $(2)$ implies
$$
unemp^{SEA}(1-u)	=u\times L_{work}^{SEA}
\Leftrightarrow unemp^{SEA}	=\frac{u}{1-u}L_{work}^{SEA}
$$
Hence, we can substitute for $unemp^{SEA}$ in equation $(1)$:
$$
1-lfp=\dfrac{L_{0}^{SEA}}{L_{0}^{SEA}+\left[\frac{1}{1-u}\right]L_{work}^{SEA}}
$$
Then,
$$
(1-lfp)\times L_{0}^{SEA}+(1-lfp)\left[\frac{1}{1-u}\right]L_{work}^{SEA}	=L_{0}^{SEA}
$$

$$
\Rightarrow -lfp\times L_{0}^{SEA}+(1-lfp)\left[\frac{1}{1-u}\right]L_{work}^{SEA}	= 0
$$
So, we conclude
$$
L_{0}^{SEA}=\frac{1-lfp}{lfp}\left[\frac{1}{1-u}\right]L_{work}^{SEA}
$$

We use this formula to calculate $L_{0}$ for each country using the unemployment rate and the labor force participation rate (both taken from ILO), and total employment, $\sum_{i=1}^{14}L_{i}$ (taken from SEA).  
```{r message=FALSE, warning=FALSE}
# Pepare lfp for each country.
