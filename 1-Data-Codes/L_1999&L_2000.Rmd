...
$$
To ease notation, here we ignore from which year we are taking each variable. However, in the exposition of Task 4 we do include a time superscript to indicate the year; the rest of the notation remains the same.
...
Also, from ILO we know the value of the unemployment rate ($u^c$) for each country, which is:
$$
...
So, we conclude
$$
L_{0}^{c,SEA}=\frac{1-lfp^c}{lfp^c}\left[\frac{1}{1-u^c}\right]L_{work}^{c,SEA}
$$

We use this formula to calculate $L^c_{0}$ for each country using the unemployment rate and the labor force participation rate (both taken from ILO), and total employment, $\sum_{i=1}^{14}L^c_{i}$ (taken from SEA).  
```{r message=FALSE, warning=FALSE}
...
Specifically, if $L_{i,s}^{2000}$ represents the employment for year 2000 of sector $s=0,...,14$ and state $i=1,...,50$ that we calculated in Task 1 (for CBS) and Task 2 (for BLS), for every $s\neq0$ we do the following normalization:
$$
...
Specifically, using the values of $L_{s}^{c,SEA,1999}$ and $L_{s}^{c,SEA,2000}$ that we calculated in Task 3 for each country $c$ (excluding USA) and each sector $s$, we do the following normalization:
$$
...
$$
where $L_{i,s}^{1999}$ represents the employment for year 2000 of sector $s=0,...,14$ and state $i=1,...,50$ that we calculated in Task 2 for BLS.
