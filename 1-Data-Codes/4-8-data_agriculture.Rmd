...
#Modifying state-country flows
This code aggregates trade flows of c million dollars from (to) state s to (from) Row if that state has no imports or exports from or towards any country. The reason for this modifications of bilateral trade flows is that each flow $$X_{ij}$$ in agriculture gravity is defined as $$X_{ij}=\tilde{\tau}_{ij}\tilde{\Pi}_{i}^{-1}\tilde{P}_{j}^{-1}Y_{i}X_{j}$$, and this definition is never zero on the right side, therefore, the constants that the gravity system multiplies on the left side can't be zero, which they were, since some states don't export or import agricultural products from countries. 
```{r}
#state-country
CENSUS.mod.1 <- CENSUS %>%
  filter(file=="i in US, j not in US") %>%
...
for (i in state.name) {
  temp <- CENSUS.mod.1 %>%
    filter(year==yr, origin==i)
  if(dim(temp)[1]==0){
    new <- cbind(i, yr, "Row", 14, 1, "i in US, j not in US")
    list <- rbind(list, new)
  }
}
}  
list <- data.frame(list)
colnames(list) <- colnames(CENSUS.mod.1)


# new.bilat <- c()
# for (i in 1:dim(CENSUS.mod.1)[1]) {
#   new <- cbind(CENSUS.mod.1$origin[i], CENSUS.mod.1$year[i], "Row", CENSUS.mod.1$sector[i], 1, CENSUS.mod.1$file[i])
#   new.bilat <- rbind(new.bilat, new)
# }
...
=======
>>>>>>> parent of c1a736f... Update 4-8-data_agriculture.Rmd
#State production 
```{r}
...
y1$sector = df.WIOD$sector[1:(num.countries*13)]
y1$importer = df.WIOD$importer_country[1:(num.countries*13)]
...
if( is.na(a[2])== TRUE) {
  for (row in 1:(dim(check)[1])) {
    if(is.na(check[row,5])== TRUE){
      r=check[row,2]
      if(r==14){r=13} #since there is gap between sec 12 and sec 14
      country=check[row,1]
      for (i in 1:37) {
...
CENSUS.2 <- melt(step3y.1, id.var=c("year", "importer"), variable.name = "origin")
CENSUS.2$origin <- as.character(CENSUS.2$origin)
write.table(CENSUS.2, file = paste("1-Intermediate_Processed_Data//census.agri.proportionality.csv", sep=""), sep = ",", row.names = FALSE)
