...
n.s <- 50
n.c <- 37
state.names <- matrix$importer[1:n.s]
c.names <- matrix$importer[(n.s+1):(n.s+n.c)]
...
#print(sum(CDP.cc$value -wiot.cc$value))
if(sum(abs(CDP.cc$value/wiot.cc$value -1 ))>epsilon) 
...
CDP.cs <- CDP %>%
  filter(year==yr, importer %in% c.names, exporter %in% state.names) %>%
...
#print(sum(CDP.cs$val -wiot.cs$value))
#temp <- data.frame(CDP.cs$val -wiot.cs$value) 
if(sum(abs(CDP.cs$val/wiot.cs$value -1))>epsilon) 
...
CDP.sc <- CDP %>%
  filter(year==yr, importer %in% state.names, exporter %in% c.names) %>%
...
#print(sum(CDP.sc$val -wiot.sc$value))
#temp <- data.frame(CDP.sc$val -wiot.sc$value)
if(sum(abs(CDP.sc$val/wiot.sc$value -1))>epsilon) 
...
CDP.ss <- CDP %>%
  filter(year==yr, importer %in% state.names, exporter %in% state.names) %>%
  group_by(sector) %>%
  mutate(val=sum(value)) %>%
  ungroup() %>%
...
#print(sum(CDP.ss$val -wiot.ss$value))
#temp <- data.frame(CDP.ss$val -wiot.ss$value) 
if(sum(abs(CDP.ss$val/wiot.ss$value -1))>epsilon) 
