...
libs <- c( 'dplyr', 'tidyr', 'readstata13', 'haven', 'stringr', 'nleqslv', 'gdata', 'Statamarkdown', 'formattable', 'ggplot2', 'viridis', 'RColorBrewer', 'reshape2', 'foreign', 'read_excel', 'readxl');
...
`1-Intermediate_Processed_Data//country_coordinates.csv`
...
city.count <- UN.0 %>% #doesn't include count for RoW
  filter(country %in% c.names) %>%
  mutate(c=1) %>%
  group_by(country) %>%
...
UN.1 <- data.frame(melt(UN.1, id.vars = c("Country.code.num", "Country.name", "country", "city", "Latitude", "Longitude"),  variable.name ="year", value.name = "city.population"))
UN.1 <- UN.1 %>% select(year, everything())
...
UN.1 <- UN.1 %>% mutate(country.name.original = Country.name, country.population.original = country.population)
...
UN.1 <- UN.1 %>%
  left_join(UN.temp, by=c('year'='year', 'country'='country')) %>%
  mutate(country.population= ifelse(country == "RoW", tot.pop.RoW, country.population)) %>%
  select(-tot.pop.RoW) %>%
  arrange(year, Country.code.num)

write.table(UN.1, file = paste("1-Intermediate_Processed_Data//country_coordinates.csv", sep=""), sep = ",", row.names = FALSE)
