...
5. `0-Raw_Data/CFS/CF1200A24.dat` (this file was too heavy, so it was compressed in a .zip folder with the same name; it must be taken out of the foler to run the script)
...
The following code assigns state-to-state trade flows from CFS to the NAICS level. We then calculate $X_{ij,k}^{CFS}$ for the 12 manufacturing sectors (where $X_{ij,k}^{CFS}$ is the sales of state $i$ to state $j$ in sector $k$ according to CFS). For simplicity, we calculate weights for 13 sectors here, and then we drop services in the last chunk to keep only our 12 manufacturing sectors.
