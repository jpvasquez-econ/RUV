...
stataexe <- "C:/Users/alove/Desktop/Stata 16/Stata16MP-64.exe"
...
1) Combine the steps of the previous 5 codes to obtain the final $X_{ij,k}^{year}$ matrix for years 2000-2007.

2) Calculate region-sector level deficits. The deficit for a region $i$ in a sector $k$ would be defined as $D_{i,k}=\sum_j X_{ij,k} - \sum_j X_{ji,k}$
...
- In a few cases, gross output might be smaller than value added (probably due to some small discrepancies between trade and production data, etc). In such cases, just constraint value added to be equal to gross output. 
...
Then we calculate gross output for each region, for each sector, using that $Y_{i,k}= \sum_j X_{ij,k} \quad for \quad all \quad j$
...
The share of value added in gross output is giving by: $Labor share = \frac{VA_{i,k}}{Y_{i,k}}$
