...
3) Calculate the share of value added in gross output for each US state, for each sector.
...
#deficits (and add the corresponding ID columns)
...
```{r}
#Create an ID for each region, and group by year to sum over sectors.
...
deficit <- deficit %>%
  ...
  select(-sector) 
#Reshape and use the ID for the previous step to arrange. 
...
We load bilateral flows to calculate gross output, state GDP, taxes, and subsidies to calculate value added for each state in the next chunk. And we load US (total) value added, which is already calculated.
```{r}
base <- c()
VA_US <- c()
GDP <- c()
