...
Xijk$sector = df$sector[1:(12*n.s)]
Xijk$importer = df$importer[1:(12*n.s)]
Xijk = Xijk %>% select(importer, sector, everything())

xijkCFS$sector = df$sector[1:(12*n.s)]
xijkCFS$importer = df$importer[1:(12*n.s)]
xijkCFS = xijkCFS %>% select(importer, sector, everything())

# Check for all  k
for(k in 1:12) {
  if(sum(abs(Xijk[Xijk$sector==k,3:52])/X_USA_USA_k[k] -1)>epsilon) 
