...
To use CFS plus proportionality to calculate $X_{ij,k}^{year}$ for $i\in US\;  \&\;   j \in US$,  $\forall k=1,...,12,14$.  I call the output of this part $X_{ij,k}^{Step2}$.
...
We should have that: $$\sum_{i} \sum_{j} X_{ij,k}^{Step3}=X_{US,US,k}^{WIOD} \quad \forall k=1,...,12,14.$$
...
relevant_sectors = 1:14

XijkCFS_1 = c()
denominator = c()
n_relevant_sectors = 14
...
# Take into account only the first 14 sectors (13 will be excluded later)
X_USA_USA_k = df_WIOD$value_USA[USA_exporter_indices]
X_USA_USA_k = X_USA_USA_k[1:14]
...
Xijk$sector = df$sector[1:700]
Xijk$importer = df$importer[1:700]
Xijk = Xijk %>% select(importer, sector, everything())

xijkCFS$sector = df$sector[1:700]
xijkCFS$importer = df$importer[1:700]
