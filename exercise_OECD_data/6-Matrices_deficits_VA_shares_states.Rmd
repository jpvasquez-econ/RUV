...
1) The goal is to combine the previous steps in order to obtain a new Final Matrix for 2015 through 2018: $X_{ij,k}^{year}$.
...
reclass_sectors <- reclass_sectors %>% 
  dplyr::rename(wiod_sector = WIOD_sector) %>%
...
The final goal is to check that the final trade matrices are consistent with ICIO database.
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#country-country  
icio <- melt(icio_base, id.vars=c("year", "importer", "sector"), variable.name="exporter", value.name = "value")
icio_cc <- icio %>%
...
if(abs(sum(cdp_cc$val)/sum(icio_cc$value) -1)>epsilon) 
  stop(paste("countries' exports to countries != countries' ICIO exports for year", yr))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(abs(sum(cdp_cs$val)/sum(icio_cs$value) -1)>epsilon) 
  stop(paste("states' exports to countries != US ICIO exports to countries for year", yr))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(abs(sum(cdp_sc$val)/sum(icio_sc$value) -1)>epsilon) 
  stop(paste("countries' exports to states != countries' ICIO exports to US for year", yr))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(abs(sum(cdp_ss$val)/sum(icio_ss$value) -1)>epsilon) 
...
deficit <- deficit %>% 
  arrange(pos) %>%
  select(-pos)
colnames(deficit) <- c("region", "deficit_2015", "deficit_2016", "deficit_2017", "deficit_2018")
...
#State GDP
gdp <- read.csv(paste("0-Raw_Data//Labor_shares//gdp_state_", yr, ".csv", sep=""), header = TRUE, sep = ",", dec = ".")
colnames(gdp) <- c("GeoFips", "GeoName", "sector", "Description", "Value")
gdp$year <- yr
gdp <- gdp %>% 
  ...
  filter(sector==3 | sector==6 | sector==11 | sector==14 | sector==15 | sector==16 | sector==17 | sector==18 | sector==19 | sector==20 | sector==21 | sector==22 | sector==23 | sector==24 | sector==26 | sector==27 | sector==28 | sector==29 | sector==30 | sector==31 | sector==32 | sector==33 | sector==34 | sector==35 | sector==37 | sector==38 | sector==39 | sector==40 | sector==41 | sector==42 | sector==43 | sector==44 | sector==45 | sector==51 | sector==56 | sector==59 | sector==69 | sector==70 | sector==76 | sector==79 | sector==82) %>%
  mutate(sector = as.numeric(recode(sector, '3'="1", '6'="2", '11'="18", '14'="6", '15'="11", '16'="12", '17'="12", '18'="13", '19'="14", '20'="14", '21'="15", '22'="15", '23'="16", '24'="16", '26'="3", '27'="4", '28'="4", '29'="6", '30'="6", '31'="2", '32'="9", '33'="10", '34'="19", '35'="19", '37'="23", '38'="23", '39'="23", '40'="23", '41'="23", '42'="23", '43'="23", '44'="34", '45'="27", '51'="28", '56'="29", '59'="34", '69'="32", '70'="33", '76'="34", '79'="22", '82'="34")))

GDP <- rbind(GDP, gdp)  
#These databases were downloaded from BEA, Regional Data section; specifically, the base is named SAGDP2. After choosing this information, select the NAICS option, All areas, All statistics in table, and each year from 2015 to 2018, separately (to download the next year just go to the previous time section window and change the selected year). Download each base in a csv file.

#State taxes
taxes <- read.csv(paste("0-Raw_Data//Labor_shares//taxes_state_", yr, ".csv", sep=""), header = TRUE, sep = ",", dec = ".")
colnames(taxes) <- c("GeoFips", "GeoName", "sector", "Description", "Value")
taxes$year <- yr
taxes <- taxes %>% 
  select(year, everything()) %>%
  filter(GeoFips <= 90000 & GeoFips != 0) %>%
...
#State subsidies
subsidies <- read.csv(paste("0-Raw_Data//Labor_shares//subsidies_state_", yr, ".csv", sep=""), header = TRUE, sep = ",", dec = ".")
colnames(subsidies) <- c("GeoFips", "GeoName", "sector", "Description", "Value")
subsidies$year <- yr
subsidies <- subsidies %>% 
  select(year, everything()) %>%
  filter(GeoFips <= 90000 & GeoFips != 0) %>%
...
GDP$GeoName <- sub(' ', '', GDP$GeoName)
TAXES$GeoName <- sub(' ', '', TAXES$GeoName)
SUBSIDIES$GeoName <- sub(' ', '', SUBSIDIES$GeoName)
...
value$sub <- SUBSIDIES$Value
colnames(value) <- c("year", "id" ,"state", "sector", "description", "gdp", "tax", "sub")
