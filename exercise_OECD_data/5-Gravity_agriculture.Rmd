...
1) This code creates a file that contains information about distances between regions, agricultural production for all regions and agricultural consumption for countries. In doing so, the code also applies the usual proportionality rule so that aggregate expenditure and production matches ICIO
...
10. Coefficients from `Country_Gravity_ICIO`
...
#   distinct(ICIO_sector, .keep_all = TRUE)
...
icio_full <- read.csv("1-Intermediate_Processed_Data//icio_full.csv")
...
data_fish[is.na(data_fish)==TRUE] <- 0
######
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
agri <- c() 
# We use Agriculture Census of 2017 
for (yr in 2015:2018) {
  num    <- data_agri %>%
...
agri$R_i_2  <- NA
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 4. APPLYING PROPORTIONALITY TO ICIO
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
## SUM OF PRODUCTIONS ACROSS STATES = TOTAL PRODUCTION OF US ACCORDING TO ICIO

##
## 4.1 TOTAL PRODUCTION OF US IN AGRICULTURE ACCORDING TO ICIO
## 
...
for (yr in 2015:2018) {
  icio_value  <- icio_agri %>%
                 filter(year == yr)
  icio_value  <- icio_value$production[1]
  agri        <- agri %>%
                 mutate(R_i_2= ifelse(year == yr, rel*icio_value, R_i_2))
}
...
##
## 5.1 Check: total production of US states = total US in ICIO
## 

# total production of US states from proportionality 
check_1     <- data_agriculture %>%
               filter(iso_o %in% s_names) %>%
...
# total production of US states from ICIO 
check_2     <- icio_full %>%
               filter(row_country == "USA", row_item == (n_sec + 2)) %>%
               group_by(year) %>%
               mutate(production = sum(value)) %>%
               ungroup() %>%
               distinct(year, .keep_all = TRUE) %>%
...
if(abs(sum(check_2$production / check_1$total-1))>epsilon)
   stop(paste("states' production does not add to ICIO for"))
...
data_agriculture <- data_agriculture %>%
  left_join(icio_p, by=c('year'='year', 'iso_o'='iso_o')) %>%
...
data_agriculture <- data_agriculture %>%
  left_join(icio_c, by=c('year'='year', 'iso_d'='iso_d')) %>%
...
for (yr in 2015:2018) {
  ...
  # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  icio_full_0 <- icio_full %>%
    filter(year == yr, col_item == 0 & col_country == "USA")
  icio_full_1 <- icio_full_0 %>%
    group_by(year, col_country) %>%
    mutate(C_us = sum(value)) %>%
    ungroup() %>%
    distinct(year, col_country, .keep_all = TRUE)
  icio_full_2 <- icio_full_0 %>%
    filter(row_item == n_sec + 2) %>%
    group_by(year, col_country) %>%
    mutate(C_us_agri = sum(value)) %>%
    ungroup() %>%
    distinct(year, col_country, .keep_all = TRUE)
  C_us <- icio_full_1$C_us
  C_us_agri <- icio_full_2$C_us_agri
  ...
  if (sum(abs(US_sum$total_R / US_totals$total - 1)) > epsilon) {
    stop(paste("sum(R_i) over states != US ICIO for year", yr))
  }
  ##
  ...
  ##
  ## checking that expenditure adds up to ICIO totals
  ##
  # expend states
  ...
  if (sum(abs(exp_states / US_totals - 1)) > epsilon) {
    stop(paste("sum(E_j) over states (except agric) != US ICIO for year", yr))
  }
  ...
  if (abs(c1 / c2 - 1) > epsilon) {
    stop(paste("sum(E_j) over states != US ICIO for year", yr))
  }
  # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
